...
topSchools_subset <- topSchools[c('SATMT75', 'ACTMT75', 'ADM_RATE', 'COMP_ORIG_YR4_RT', "PELL_COMP_ORIG_YR6_RT", 'MN_EARN_WNE_INC1_P10')]
...
topSchools_subset1$MN_EARN_WNE_IC1_P10 <- as.numeric(as.character(topSchools_subset$MN_EARN_WNE_INC1_P10))
...
trainset_subset <- trainset[c('SATMT75', 'ACTMT75', 'ADM_RATE', 'COMP_ORIG_YR4_RT', 'PELL_COMP_ORIG_YR6_RT','MN_EARN_WNE_INC1_P10')]
...
testset_subset <- trainset[c('SATMT75', 'ACTMT75', 'ADM_RATE', 'COMP_ORIG_YR4_RT','PELL_COMP_ORIG_YR6_RT','MN_EARN_WNE_INC1_P10')]
...
set.seed(12345)
cv.out1 <- cv.glmnet(x_train_scaled, y_train_scaled, alpha=0, lambda=grid, intercept=FALSE, standardize=FALSE)
...
save(cv.out1, coeff_ridge1, bestlambda1, full_mse_ridge1, ridge1cor, ridge_full1, file='data/rigde1-reg.RData')


#Plot the cross-validation errors
png("images/ridgeCV_lowest.png")
plot(cv.out1)
dev.off()
