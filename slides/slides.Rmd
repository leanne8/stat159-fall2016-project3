...
We use the dataset CollegeScore Card from https://collegescorecard.ed. gov/data/. In data-subsetting.R, we subset 34 columns that we need for the project. In data-cleaning.R, there’s a lot of null data and privacy suppressed data so we have to delete those rows and replace some with N/A. After cleaning the data we put them in different csv files.

In the pre-processing.R we select schools with top SAT math and ACT scores that are at least one standard deviation above average, and then we turn those columns that we need into numerics from factors. We decided on 5 predictors and 1 dependent variable out of the 34 options. Finally, we export the clean dataset into a topschool.csv.
